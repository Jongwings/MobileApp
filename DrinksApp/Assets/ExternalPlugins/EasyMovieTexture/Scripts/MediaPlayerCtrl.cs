...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    {
		if (System.IO.Directory.Exists(Application.persistentDataPath + "/StreamingMedia") == true)
			System.IO.Directory.Delete(Application.persistentDataPath + "/StreamingMedia", true);
    }
    ...
    void OnError(MEDIAPLAYER_ERROR iCode, MEDIAPLAYER_ERROR iCodeExtra)
    {
        ...
        Q.Utils.QDebug.LogError(strError);
    ...
    {
		Q.Utils.QDebug.Log("ApplicationPause : " + bPause);
		Q.Utils.QDebug.Log("m_CurrentState : " + m_CurrentState);
		if (bPause == true) {
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
		...
		if( strFileName.Contains("://") == false)
		{
			...
			Q.Utils.QDebug.Log(strFileName);
		}
		...
		for (var i = 0; i < (pFormatContext)->nb_streams; i++)
		{
			if ((pFormatContext)->streams[i]->codec->codec_type == AVMediaType.AVMEDIA_TYPE_VIDEO)
			{
				if(bFindVideo == false)
				{
					...
					Q.Utils.QDebug.Log("Video" +  iStreamIndex);
				}
	...
	{
		Q.Utils.QDebug.Log("EasyMovieTexture: " + message);
	}
	...
	private void UpdateVideo()
	{
		...
		if( m_CurrentState != MEDIAPLAYER_STATE.PAUSED)
		{
           ...
           // Q.Utils.QDebug.Log(pPacket->pts + " " + pCodecContext->pts_correction_last_pts );
							...
							if (iAudioLen >= 0) {
								if (gotSound == 1) {
                                    ...
                                    if( pAudioCodecContext->sample_fmt != AVSampleFormat.AV_SAMPLE_FMT_FLT)
                                    {
                                        //for(int i = 0; i < pAudioCodecContext->channels; i++)
                                        {
                                            ...
                                            if((error = ffmpeg.swr_init(pAudioCvtContext)) < 0) {
                                                Q.Utils.QDebug.Log ("error " + error);
                                            }
									...
									if( bSeekTo == true)
									{
										...
										//Q.Utils.QDebug.Log(value + " " + pts + " " + GetDuration() + " " + pDecodedAudioFrame->pkt_duration);

										iSoundCount = (int)value;
										bSeekTo = false;
									}

									//Q.Utils.QDebug.Log ("sound " + iSoundCount);
                                            //Q.Utils.QDebug.Log (pDecodedAudioFrame->pkt_dts + " " +pDecodedAudioFrame->pkt_duration + " "  + pDecodedAudioFrame->pkt_pos + " " + pDecodedAudioFrame->pkt_pts + " " + pts);
                                            ...
                                            lock( listAudio)
                                            {
                                                ...
                                                {       
                                                    {
														//Q.Utils.QDebug.Log(pDecodedAudioFrame->pkt_pts % pDecodedAudioFrame->pkt_duration + " " + iDataSize2);
                                            ...
                                            //Q.Utils.QDebug.Log ( iDataSize + " "+ pts + " " + pAudioCodecContext->sample_rate + " " + pDecodedAudioFrame->sample_rate + " "+ pDecodedAudioFrame->nb_samples + " " + pDecodedAudioFrame->pkt_pts + " " + pDecodedAudioFrame->pkt_pos);
                                            //Q.Utils.QDebug.Log ("sound decode time " + pts);
                                    ...
                                    else
                                    {
                                        ...
                                        //Q.Utils.QDebug.Log ( pDecodedAudioFrame->channels + " "+buffer.Length / 4 + " " + pDecodedAudioFrame->pkt_pts + " " + pDecodedAudioFrame->pkt_pos);
                                        //Q.Utils.QDebug.Log ("sound decode time " + pts);
    ...
    //Q.Utils.QDebug.Log("last " + fLastFrameTime + " " + fCurrentSeekTime + " " + listVideo.Count);
                ...
                if (audioSource != null)
                {
					...
					if (listAudioPts [i] >= 0) {
						
						{
							...
							{
								//Q.Utils.QDebug.Log(audioSource.time  + " " + (float)listAudioPtsTime[i]  + " " +listAudioPts [i]  );
		...
		else
		{
            ...
            if( audioSource != null)
            {
                ...
                {
                    //Q.Utils.QDebug.Log("sync1 " + audioSource.time + " " + fLastFrameTime);
                    audioSource.time = fLastFrameTime;
                }
                else if( audioSource.time - fLastFrameTime < -0.2f)
                {
                    //Q.Utils.QDebug.Log("sync2");
                    audioSource.time = fLastFrameTime;
                }
            }
            ...
            if (audioSource != null)
           {
				if (listAudioPts [i] >= 0) {
					
					{
						...
						{
							//Q.Utils.QDebug.Log(pAudioCodecContext->sample_rate + " " +listAudioPts [i]  );
							audioClip.SetData(listAudio[i],(int)(listAudioPts[i]  )  );
						}
					}
				}
            ...
            //Q.Utils.QDebug.Log(listAudio.Count);
...
*/
  ...
  */      
		...
		//Q.Utils.QDebug.Log (fLastFrameTime + " " + fCurrentSeekTime);
	...
	private int Call_GetVideoWidth()
	{
		//Q.Utils.QDebug.Log (m_iWidth);
		return m_iWidth;
	}
	
	private int Call_GetVideoHeight()
	{
		//Q.Utils.QDebug.Log (m_iHeight);
		return m_iHeight;
	}
    ...
    public IEnumerator DownloadStreamingVideoAndLoad(string strURL)
    {
        ...
        Q.Utils.QDebug.Log("DownloadStreamingVideo : " + strURL);
        ...
        if (string.IsNullOrEmpty(www.error))
        {
			...
			{
				Q.Utils.QDebug.Log("Delete : " + write_path);
    ...
    IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
    {
        ...
        {
            Q.Utils.QDebug.Log("CopyStreamingAssetVideoAndLoad : " + strURL);
