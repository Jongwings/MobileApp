...
public class SearchIngredientsPanelController : MonoBehaviour {
	...
	public void CollectionsAPICalls()
	{
		...
		string url = AppServerConstants.BaseURL+AppServerConstants.Search_Ingredients_Recipe;
	...
	IEnumerator CollectionServerCallback (WWW www)
	{
		...
		if (www.error == null) {
			...
			List<SeralizedClassServer.SearchPanelIngredientsList> categoryList = new List<SeralizedClassServer.SearchPanelIngredientsList> ();
			categoryList = JsonConvert.DeserializeObject<List<SeralizedClassServer.SearchPanelIngredientsList>> (temp);
	...
	void DisplayCollectionBrands(List<SeralizedClassServer.SearchPanelIngredientsList> collectionBrandList)
	{
		GameObject[] collectionBrandGameObject = new GameObject[collectionBrandList.Count];
		int count = 0;
		foreach(SeralizedClassServer.SearchPanelIngredientsList brand in collectionBrandList)
		{
			...
			collectionBrandGameObject [count].transform.GetComponent<SearchIngredientsRecipes> ().InitBrand (brand);
			count++;
		}
	}
